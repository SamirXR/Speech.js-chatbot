...
function strTrim(strng){
    ...
    if( strng.charAt( loc ) == " "){
        ...
        var flg = false;
        for(var i=0; i<=5; i++ ){ flg = flg || ( strng.charAt( loc ) == punct[i]); }
        if(flg){	
            aString = strng.substring( - ( ht - 1 ), strng.length - ht );
        } else { aString = strng; }
...
function conjugate( sStrg ){           	// rephrases sString
    var sString = sStrg;
    for( var i = 0; i < maxConj; i++ ){			// decompose
...
var sInput,Rtimer,wInput,lTopic;

function listeneliza(User){
    sInput = User;
